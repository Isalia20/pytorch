...
static Tensor& multinomial_with_replacement_mps_kernel(const Tensor& self,
  ...
  // Inverse CDF sampling: cumsum for the CDF, then binary search.
  // O(n) memory instead of the previous O(n^2) matmul approach that
  // allocated a [numCategories x numCategories] intermediate matrix.
  auto total = self_v.sum(-1, /*keepdim=*/true);
  auto cdf = self_v.cumsum(-1);
  cdf.div_(total);

  auto uniform = at::empty(result_v.sizes(), self.options());
  uniform.uniform_(0.0, 1.0, generator);

  result_v.copy_(at::searchsorted(cdf, uniform));
  result_v.clamp_(0, static_cast<int64_t>(numCategories) - 1);
